...
public class BaseDaoImpl<T extends Serializable, PK extends Serializable>
    ...
    //根据搜索条件查询实体List
    public List queryEntityListByConditions(final String hql, final Integer limit,
			final Integer start, final Object... conditions) {
    ...
    public void deleteEntityById(final String entityName, final int id) {
    	this.getHibernateTemplate().execute(new HibernateCallback<T>() {
			...
			public T doInHibernate(Session session) throws HibernateException,
					SQLException {
				Query query = session.createQuery("delete from " + entityName + " as o where o.=?");
  ...
  //根据id值删除指定实体
    public void deleteEntityById(final String entityName, final String idName, final int id) {
    	this.getHibernateTemplate().execute(new HibernateCallback<T>() {

			@Override
			public T doInHibernate(Session session) throws HibernateException,
					SQLException {
				Query query = session.createQuery("delete from " + entityName + " as o where o." + idName + "=?");
				query.setParameter(0, id);
				query.executeUpdate();
				return null;
			}
    ...
    //根据装载实体属性的Map查询实体
	@Override
	public T queryEntityByProperties(Class<T> entityClass, Map<String, Object> propertyMap) {
	...
	//构造hql查询语句
	private String createHqlFromProtitiesMap(Class entityClass, Map<String, Object> propsMap) {
		StringBuilder hqlBuilder = new StringBuilder("from ");
		String entityName = entityClass.getSimpleName();
		hqlBuilder.append(entityName);
		hqlBuilder.append(" as o");
