...
<script>
	...
	function getMoreDic(page){
		var rows = 100;
		var type  = $('#typeValue').val();
		var urlStr = 'struts/ViewDic_getMoreDicItems?codeName='+type+"&rows="+rows+"&page="+page;
		$.ajax({
			url:encodeURI(urlStr),
			success: function(data){
				var jsonObject = eval('('+data+')');
				...
				$.each(lines,function(n,value){
					valueStr += "<li><a href=\"viewDicItem.jsp?type="+$('#typeValue').val()+"&key="+value.showName+"\">"+value.showName+"</a></li>";
				});
				if(valueStr == '')
